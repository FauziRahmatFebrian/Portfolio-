...
const ChatAI = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyBEhuZ0W1BpD3Vf6jj0TNQMSvkPEd_DgiY`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              ...
              Jawab dengan ramah dan informatif dalam bahasa Indonesia. Fokus pada informasi yang tersedia tentang Fauzi.`
            }]
          }]
        })
      });
      ...
      const aiResponse = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Maaf, saya tidak dapat menjawab pertanyaan tersebut saat ini.';
